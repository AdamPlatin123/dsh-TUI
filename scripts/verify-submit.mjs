...
// cancel the same driver again or double-deliver. The channel ignores the
// second request until the abort's terminal event clears its latch.
let resolveIdle2
...
resolveIdle2()
await sleep(10)
check('double interrupt does not double-cancel or double-deliver', interruptFollowups.filter(m => m.content?.[0]?.text === 'x').length === 1 && interruptFollowups.filter(m => m.content?.[0]?.text === 'y').length === 0, JSON.stringify(interruptFollowups.map(m => m.content?.[0]?.text)))
