 ...
 */
const { upstreamDrift, UPSTREAM_VALIDATED_VERSIONS } = await import('../src/dsh-adapter/contract.js')

const drift = upstreamDrift()
if (drift.length > 0) {
  console.error(`Upstream contract violated (validated: ${UPSTREAM_VALIDATED_VERSIONS.join(' / ')}):`)
  for (const entry of drift) {
    console.error(`  - ${entry.package}: installed=${entry.installed ?? 'missing'}`)
  }
  process.exit(1)
}
console.log(`upstream contract OK (validated: ${UPSTREAM_VALIDATED_VERSIONS.join(' / ')})`)
